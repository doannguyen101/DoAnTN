...
$(document).ready(function () {
    ...
    modalList.on('click', '.extent-trial', function (e) {
        ...
        $.post('/su-kien/khoa-co-ban-10/extent-trial.php', {freeLearnId: freeLearnId}, function (response) {
            ...
            if (response.status == 'success'){
                alert.find('.modal-title').text('ThA nh cA¥ng');
                alert.find('.warning-des').html(response.message);
                alert.modal('show');
                expiredButton.removeClass('extent-trial').attr('href', response.url).attr('target', '_blank').unbind('click').text('H·ªçc ngay');
                expiredButton.parents('tr').find('.expired').html(response.time);
            } else {
                alert.find('.modal-title').text('L·ªói gia h·∫°n');
                alert.find('.warning-des').html(response.message);
                alert.modal('show');
            }
        }, 'JSON');
                ...
                if(response.count) {
                    ...
                    for (var i in response.registedCourses) {
                        ...
                        if (response.registedCourses[i].isExpired) {
                            button += '<a class="extent-trial btn btn-fl" href="#" data-id="'+ i +'">Gia h·∫°n</a>';
                        } else {
                            button += '<a class="btn btn-fl" href="'+ response.registedCourses[i].url +'" data-id="'+ i +'">H·ªçc ngay</a>';
                        }
                    ...
                    tableBody.append('<tr><td colspan="4"><div class="info">B·∫°n ch∆∞a ƒëƒÉng kAΩ khA≥a h·ªçc mi·ªÖn phA≠ nA o<br/>Vui lA≤ng ch·ªçn khA≥a h·ªçc bA™n d∆∞·ªõi!</div></td></tr>');
                }
                        ...
                        if (typeof response.unregistedCourses[i] !== 'string') {
                            ...
                            for (var j in response.unregistedCourses[i]) {
                                ...
                                var package = '<tr>' +
                                    ...
                                    '<td style="text-align: center"><a target="_blank" href="'+ response.unregistedCourses[i][j].url +'" data-package="'+ j +'" class="btn btn-primary regist-course">H·ªçc mi·ªÖn phA≠ khA≥a nA y</a>';

                                    if (response.unregistedCourses[i][j].upgradeUrl) {
                                        package += '<a style="margin-left: 5px" target="_blank" href="'+ response.unregistedCourses[i][j].upgradeUrl +'" data-package="'+ j +'" class="btn btn-success regist-course">NA¢ng c·∫•p</a>';
                                    }
    ...
    $('.learntrial-form').validate({
        ...
        messages: {
            code: {
                required: 'B·∫°n ch∆∞a nh·∫≠p mA£ code'
            },
        },
        errorElement: 'div',
                    ...
                    success: function ($response) {
                        switch($response.status){
                            case 'success':
                                ...
                                $mModal.find('.modal-title').html('XA°c nh·∫≠n thA nh cA¥ng');
                                $mModal.find('.modal-footer').html('<button type="button" class="btn btn-cancel" data-dismiss="modal">ƒêA≥ng</button>');
                                $mModal.modal('show');
                            break;
                            case 'fail':
                                var $mModal = $('#messageModal');
                                $mModal.find('.warning-des').html($response.message);
                                $mModal.find('.modal-title').html('L·ªói');
                                $mModal.find('.modal-footer').html('<button type="button" class="btn btn-cancel" data-dismiss="modal">ƒêA≥ng</button>');
                                $mModal.modal('show');
                            break;
