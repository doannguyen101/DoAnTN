...
function show_message($title, $message, $footer){
    ...
    if(typeof $footer == 'undefined'){
        $mModal.find('.modal-footer').html('<button type="button" class="btn btn-cancel" data-dismiss="modal">ĐA�ng</button>');
    }else{
...
$(document).ready(function(){
    ...
    if ($('.course-detail-description').height() > 130) {
        var $moreDetail = $('<a href="#" class="cdd-more-bt">ThA�ng tin chi tiết &raquo;</a>');
    ...
    if($relateCourses.length > 5){
        ...
        var $rlMore = $('<div class="relate-course-more">Xem thA�m</div>');
...
//        }
    ...
    issueForm.validate({
        ...
        messages:{
            link: {required: 'Link khA�ng hợp lệ'},
            relate: {required: 'Bạn chưa chon vấn đề'},
            issue: {required: 'Bạn chưa chọn lỗi'},
            email: {required: 'Mail khA�ng hợp lệ', email: 'Mail khA�ng hợp lệ'},
            phone: {required: 'Bạn chưa nhập số điện thoại', phone: 'Số điện thoại khA�ng hợp lệ'},
            title: {required: 'Bạn chưa chọn tiA�u đề'},
            time: {required: 'Thời gian khA�ng hợp lệ'},
        ...
        if($this.hasClass('with-play-scorm')){
            if(typeof $loadvideo != 'undefined'){
                if($this.hasClass('confirm')){
                    //hiểm thị thA�ng bA�o trừ số lần xem
                            ...
                            success: function($response) {
                                ...
                                if($jdata){
                                    ...
                                    if($jdata.status == 'success'){
                                        ...
                                        show_message('NA�ng cấp hệ thống', $jdata.message);
                                    }
                                }else{
                                    alert('Vui lA�ng liA�n hệ Hotline: 1900 6933');
                                }
                        ...
                        success: function($res){
                            if($res.login == 'true'){
                                ...
                                if ($res.exeaction == 'doTest') {
                                    ...
                                    //Nếu lA  hiện thA�ng bA�o
                                    if($res.showmess){
                                        $mModal.find('.warning-des').html($res.message);
                                        if($res.messaget){
                                            $mModal.find('.modal-title').html($res.messaget);
                                        }else{
                                            $mModal.find('.modal-title').html('ThA�ng bA�o');
                                        }
                                        if($res.messagef){
                                            $mModal.find('.modal-footer').html($res.messagef);
                                        }
                                        ...
                                        if($res.exeaction){
                                            switch($res.exeaction){
                                                case 'freeLearnBooking':
                                                    $modalIC.data('title', 'Cập nhật thA�ng tin đăng kA� học miễn phA�');
                                                ...
                                                default:
                                                    if(typeof $freescorm != 'undefined' && $freescorm){
                                                        $modalIC.data('title', 'Cập nhật thA�ng tin để học miễn phA�');
                                                    }else{
                                                        $modalIC.data('title', 'Cập nhật thA�ng tin để vA o học');
                                                    }
    ...
    $('#playscromvideo').click(function(){
        $('.scorm-header.with-play-scorm').removeClass('confirm').trigger('click');
        //Nếu chọn khA�ng hiển thị thA�ng bA�o nữa
    ...
    $('body').on('click', '.extent-trial2', function(){
        ...
        $.post('/su-kien/khoa-co-ban-10/extent-trial.php', {freeLearnId: freeLearnId}, function (response) {
            ...
            if (response.status == 'success'){
                alert.find('.modal-title').text('Gia hạn thA nh cA�ng');
                alert.find('.warning-des').html(response.message);
                alert.find('.modal-footer').html('');
                alert.modal('show');
            } else {
                alert.find('.modal-title').text('Gia hạn chưa thA nh cA�ng');
                alert.find('.warning-des').html(response.message);
                alert.find('.modal-footer').html('');
                alert.modal('show');
            }
