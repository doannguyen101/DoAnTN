var mootabs=new Class({initialize:function(element,options){this.options=Object.extend({width:'300px',changeTransition:Fx.Transitions.Bounce.easeOut,duration:1000,mouseOverClass:'active',activateOnLoad:'first',useAjax:false,ajaxUrl:'',ajaxOptions:{method:'get'},ajaxLoadingText:'Loading...'},options||{});this.el=$(element);this.elid=element;this.el.setStyles({width:this.options.width});this.titles=$$('#'+this.elid+' ul li');this.panels=$$('#'+this.elid+' .mootabs_panel');this.titles.each(function(item){item.addEvent('click',function(){item.removeClass(this.options.mouseOverClass);this.activate(item);}.bind(this));item.addEvent('mouseover',function(){if(item!=this.activeTitle)
...
if($type(tab)=='element')
{var newTab=tab.getProperty('title');this.panels.removeClass('active');this.activePanel=this.panels.filterById(newTab)[0];this.activePanel.addClass('active');this.titles.removeClass('active');tab.addClass('active');this.activeTitle=tab;if(this.options.useAjax)
{this._getContent();}}},_getContent:function(){this.activePanel.setHTML(this.options.ajaxLoadingText);var newOptions={update:this.activePanel.getProperty('id')};this.options.ajaxOptions=Object.extend(this.options.ajaxOptions,newOptions||{});var str=this.options.ajaxUrl;if(str.search(/\?/)!=-1){var tabRequest=new Ajax(this.options.ajaxUrl+'&tab='+encodeURIComponent(this.activeTitle.getProperty('title')),this.options.ajaxOptions);}else{var tabRequest=new Ajax(this.options.ajaxUrl+'?tab='+encodeURIComponent(this.activeTitle.getProperty('title')),this.options.ajaxOptions);}
...
{newTitle.removeClass(this.options.mouseOverClass);}}.bind(this));var newPanel=new Element('div',{'id':title,'class':'mootabs_panel'});if(!this.options.useAjax)
